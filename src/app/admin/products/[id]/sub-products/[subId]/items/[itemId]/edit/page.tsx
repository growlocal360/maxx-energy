...
export default function EditProductItemPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [productRes, subProductRes, itemRes] = await Promise.all([
      ...
      if (itemRes.ok) {
        const item: ProductItem = await itemRes.json();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (response.ok) {
      ...
      alert(error.error || "Failed to update product item");
    }
